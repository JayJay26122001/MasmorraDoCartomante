Shader "Unlit/ShaderBase"
{
    ...
    SubShader 
    {
        ...
        Pass 
        {
            ...
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS _MAIN_LIGHT_SHADOWS_CASCADE _MAIN_LIGHT_SHADOW
            ...
            v2f vert (appdata v)
            {
                ...
                o.staticLightmapUV);
                #ifdef DYNAMICLIGHTMAP_ON
                v.dynamicLightmapUV = v.dynamicLightmapUV.xy *
                unity_DynamicLightmapST.xy + unity_Dy
                #endif
            ...
            InputData createInputData(v2f i, float3 normalTS)
            {
                ...
                i.tangentWS.xyz);
                inputData.tangentToWorld = float3x3(i.tangentWS.xyz,
                bitangent, i.normalWS);
                inputData.normalWS = TransformTangentToWorld(normalTS,
                inputData.tangentToWorld);
                inputData.normalWS =
                NormalizeNormalPerPixel(inputData.normalWS);
                ...
                i.dynamicLightmapUV, i.vertexSH, i
                #else
                inputData.bakedGI = SAMPLE_GI(i.staticLightmapUV,
                i.vertexSH, inputData.normalWS);
                #endif
                inputData.normalizedScreenSpaceUV =
                GetNormalizedScreenSpaceUV(i.positionCS);
                inputData.shadowMask =
                SAMPLE_SHADOWMASK(i.staticLightmapUV);
                return inputData;
            }
            SurfaceData createSurfaceData(v2f i)
            {
                ...
                // Emission output.
                #if USE_EMISSION_ON
                surfaceData.emission = tex2D(_EmissionTex, i.uv) *
                _EmissionColor;
                #endif
