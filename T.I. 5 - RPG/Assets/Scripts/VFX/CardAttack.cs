...
public class CardAttack : MonoBehaviour
{
    ...
    public void BezierCurve()
    {
        ...
        aux = new Vector3(startPos.x + (targetPos.x - startPos.x) / 2, Mathf.Clamp(startPos.y + Mathf.Abs((targetPos.z - startPos.z) / 2) + Mathf.Abs((targetPos.x - startPos.x) / 2), 0, 25), startPos.z + (targetPos.z - startPos.z) / 2);
        t = 0;
        gameObject.SetActive(true);
        CurveMovement();
    }
    ...
    void CurveMovement()
    {
        if(t < 1)
        {
            ...
            t = Mathf.Clamp(t + Time.deltaTime * 1.5f, 0, 1);
